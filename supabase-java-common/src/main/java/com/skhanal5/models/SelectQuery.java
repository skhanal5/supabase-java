...
/**
 * Represents a query that is used to select/search rows from a Supabase Database table. <br>
 * <br>
...
public class SelectQuery implements Query {
  ...
  /**
   * The main entrypoint to build out a SelectQuery instance based on the different settings you
  ...
  public static class SelectQueryBuilder {
    ...
    /**
     * Used to specify how many rows we want to include in our query. This is 0-based indexing
     * (i.e., range(0,3) would include the first 4 results)
     *
    ...
    public SelectQuery build() {
      return new SelectQuery(
          table, columnsToSelect, Optional.ofNullable(pagination), Optional.ofNullable(filter));
    }
  }

  /**
   * Converts this SelectQuery into query parameters, so we can form a proper API call with the
   * correct parameters.
   *
  ...
  /**
   * In case range() is invoked, this method will add a header to the WebClient that will include
   * pagination.
   *
