...
/**
 * Represents a query that is used to insert rows from a Supabase Database table. <br>
 * <br>
 * For convenience, use the builder that is provided to provide the details of what table and rows
 * you want to insert from.
 *
...
public class InsertQuery implements Query {
  ...
  /**
   * The main entrypoint to build out a InsertQuery instance based on the different settings you
   * want to configure in your query.
   */
  private InsertQuery(
      @NonNull String table, @NonNull List<Map<String, Object>> valuesToInsert, Boolean select) {
    this.table = table;
  ...
  public static class InsertQueryBuilder {
    ...
    /**
     * Used in case the consumer wants to get the response back from the API with the inserted
     * contents
     *
     * @return a InsertQueryBuilder with this configured
     */
  ...
  /**
   * Converts this InsertQuery into query parameters, so we can form a proper API call with the
   * correct parameters.
   *
